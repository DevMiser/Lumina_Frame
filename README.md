...
<img width="998" height="199" alt="image" src="https://github.com/user-attachments/assets/0e5699a2-45de-4ef8-9381-86d99fb34b89" />
...
**Waveshare 8-inch DSI LCD Display** — The [Waveshare 8-inch Display](https://www.waveshare.com/8inch-DSI-LCD-C.htm) is the one Lumina and its 3D-printable frame are designed for. It connects directly to the Raspberry Pi 4 via the DSI ribbon cable connector.
...
## Create a Google Account and Obtain Your Gemini API Key *(Optional - Nano Banana script only)*

> **Note:** This step is only required if you intend to use `Lumina_Frame_Nano_Banana.py`. If you plan to use `Lumina_Frame_GPT_Image_2.py`, skip this section.
...
Lumina Frame loads its API keys from a `.env` file located in the same folder as the script. Create this file by entering the following commands:
...
Add the following lines to the file, replacing the placeholder text with your actual keys:
...
> **Note:** The `GOOGLE_API_KEY` line is only required if you are using `Lumina_Frame_Nano_Banana.py`. If you are using `Lumina_Frame_GPT_Image_2.py`, you may omit that line entirely.
...
Move the Lumina keyword files to the Porcupine raspberry-pi keywords folder by opening a terminal and entering the following commands:
...
Important — Note there are two blank spaces in each of the above commands — one between "mv" and "/home" and one between ".ppn" and "/home". Be sure to include them. There are no other spaces between the letters.

---

## Run the Program

Make sure your Waveshare DSI display is connected and your USB speakerphone is plugged in. Then open a terminal, navigate to the Lumina folder, and activate the virtual environment:
...
Then run the script you want to use:

**Nano Banana version** (Google Gemini Nano Banana 2 image generation):
```
python Lumina_Frame_Nano_Banana.py
```

**GPT Image 2 version** (OpenAI GPT Image 2 image generation and editing):
```
python Lumina_Frame_GPT_Image_2.py
```
...
Lumina will acknowledge your request, generate the image, and display it on the screen. The image orientation (landscape, square, or portrait) is chosen automatically based on your display's resolution. When using `Lumina_Frame_Nano_Banana.py`, images are generated by Google Gemini Nano Banana 2. When using `Lumina_Frame_GPT_Image_2.py`, images are generated by OpenAI's GPT Image 2.

### Editing Images *(GPT Image 2 script only)*

If you are using `Lumina_Frame_GPT_Image_2.py`, you can edit a previously generated image by voice without regenerating it from scratch. Each edit builds on the currently displayed image, so you can refine it iteratively. For example:

*Make the sky more dramatic.*

*Add a full moon to the scene.*
...
*Give everything a watercolor style.*

Lumina will acknowledge your request, apply the edit, and display the updated image. If no image is currently displayed, Lumina will let you know.
...
Add the content that matches the script you want to autostart:
...
Type=Application
Name=Lumina Frame
Exec=/bin/bash -c 'cd /home/pi/Lumina && /home/pi/Lumina/venv/bin/python /home/pi/Lumina/Lumina_Frame_Nano_Banana.py'
...
Type=Application
Name=Lumina Frame
Exec=/bin/bash -c 'cd /home/pi/Lumina && /home/pi/Lumina/venv/bin/python /home/pi/Lumina/Lumina_Frame_GPT_Image_2.py'
X-GNOME-Autostart-enabled=true
```
...
The following constants near the top of either script can be adjusted to suit your setup:
...
```
Lumina/
├── Lumina_Frame_Nano_Banana.py      # Main program — Nano Banana 2 image generation
├── Lumina_Frame_GPT_Image_2.py      # Main program — GPT Image 2 image generation and editing
...
Trixie uses Wayland by default, which is fully supported. Confirm that the `/sys/class/backlight/10-0045/brightness` sysfs path exists by running `ls /sys/class/backlight/`. If the folder name differs, update the `_backlight_path()` function in your script with the correct name.
...
**Image editing fails** *(GPT Image 2 script only).*
Verify your OpenAI API key is valid and that your account has sufficient credits. Confirm that an image has been generated in the current session — editing is not available if no image is currently displayed.
...
**Lumina Frame does not launch at startup (if you have set it to do so).**
Confirm the autostart file exists: `ls ~/.config/autostart/lumina.desktop`. If it is missing, re-create it following the steps in the autostart section above, making sure the script name in the `Exec` line matches the script you want to run. To check whether the script is currently running, enter `pgrep -a python` in a terminal.
