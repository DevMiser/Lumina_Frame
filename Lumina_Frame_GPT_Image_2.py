...
LUMINA = ("""
    ...
    # Preambles
    - When calling a tool that may take a moment (weather, image generation, image recall),
      give a brief one-sentence spoken acknowledgment before the tool executes.
    - Keep preambles to one short sentence. Do not use filler phrases like "Let me think..."
      or "One moment while I process..."
...
INACTIVITY_TIMEOUT = 2   # Oracle times out after this many seconds of inactivity
...
#        self.screen = pygame.display.set_mode((self.WIDTH, self.HEIGHT), pygame.DOUBLEBUF | pygame.OPENGL)
    ...
    def _wrap_text_to_width(self, text, font, max_width_px):
        """Word-wrap `text` so each rendered line fits within max_width_px.

        Uses font.size() to measure actual pixel width — accurate for
        proportional fonts like Arial Black. Returns a list of line strings.

        Words that are themselves wider than max_width_px (e.g., long file
        paths with no spaces) are broken on natural separators (/, _, -, .)
        first, falling back to character-level breaks only if a single
        sub-piece is still too wide.
        """
        words = text.split()
        if not words:
            return []
        ...
        for word in words:
            ...
            for piece in self._break_long_word(word, font, max_width_px):
                # Long-word pieces concatenate without an inserted space, so
                # paths like /home/pi/Lumina render as one continuous string
    ...
    def _break_long_word(self, word, font, max_width_px):
        ...
        Prefers breaks after natural separator characters (/, _, -, .).
        If a single resulting piece is still too wide, falls back to
        character-by-character breaks for that piece only.
        """
        BREAK_CHARS = "/_-."
        ...
        # Second pass: greedily pack pieces into chunks that fit. Any piece
        # that is itself wider than the column gets char-level fallback.
        chunks = []
        cur = ""
        for piece in pieces:
    ...
    def display_text(self):
        """Render title + body centered vertically with narrow margins.

        - Title (line1) is centered horizontally; serves as a header.
        - Body (line2) is word-wrapped to fit a centered column (80% of
          screen width) and rendered left-aligned within that column.
        - The whole title+body block is vertically centered between
          top/bottom margins (5% of screen height each).
        - If the body would overflow the available height, trailing lines
          are dropped and an ellipsis is appended to the last visible line.
        """
        self.ui_surface.fill(self.BACKGROUND)
...
class Realtime:
    def __init__(self, api_key, ws_url, p_instance, visualizer_instance, porcupine):
        ...
        # Silent-dismissal of false wake-word triggers — set when the model
        # calls the dismiss_false_wake tool. The main loop watches this
        # flag and closes the session quietly when it flips True.
    ...
    def _silently_dismiss(self, reason="unknown"):
        """Mark this session for silent close (model called dismiss_false_wake).

        Cancels any in-flight response and clears the local playback buffer
        so no audio leaks through before the session closes. The outer main
        loop sees `dismissed_silently` on its next tick and breaks out.
        """
    ...
    def on_msg(self, msg):
        ...
        # Early-cancel: catch dismiss_false_wake the instant it appears in the
        # response, before any audio the model may emit alongside it has had
        # time to reach the speaker. _silently_dismiss cancels the response
        # and clears the local playback buffer.
    ...
    def execute_tool(self, call_id, func_name, raw_args):
        ...
        # Defensive: if our early-cancel didn't beat the function_call_arguments.done
        # event, catch dismiss_false_wake here. No output is sent back to the model
        # because we're closing the session; the main loop will see the flag.
                ...
                if keyword_index == 0:
                    ...
                    try:
                        ...
                        if dismissed_silently:
                            # Silent dismissal of a false wake — return the screen
                            # to its pre-wake state, exactly like the post-inactivity
                            # default path. No last_interaction_time update: a false
                            # wake should not reset the screen-blank countdown.
