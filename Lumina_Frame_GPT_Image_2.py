...
LUMINA_BASE = ("""
    # Role & Identity
    - Your name is LooMa-Na Frame.
    - You are a personable and helpful assistant.
    - You do not do transcriptions.
    ...
    - Do not start your response with 'sure', 'good question', 'great', 'absolutely', or any other obsequious statement.
    - Do not end your response with a question unless necessary for clarification.

    # Appearance
    - You have a visual display that shows a glowing orb that pulses in sync to what you say.
    ...
    # Image Editing
    - After an image is displayed, you can modify it when the user asks to change, adjust,
      edit, or modify it (e.g., "make the sky more orange", "add a cat", "remove the tree").
    - Call the edit_image tool with a prompt describing what should change.
    - Each edit builds on the current image, so the user can refine iteratively.
    - If no image is currently displayed, let the user know there is nothing to edit.

    # Image Saving
    - You can save the most recently generated image when the user asks to save, keep, or store it.
    - When saving, call the save_image tool and then announce that the image has been saved.
    ...
    - By default, call tools SILENTLY and speak only the result. Never say
      ...
      look that up", or similar before tool calls.
    - This silent default applies to: get_current_time, get_current_weather,
      get_weather_forecast, set_timer, cancel_timer, get_timer_status,
      set_volume, save_image, recall_image, list_saved_images, show_screen,
      show_logo, set_always_on, set_interaction_mode, and dismiss_false_wake.
      Just call the tool and speak the result.
    - The ONLY tools that get a brief one-sentence acknowledgment BEFORE
      they run are:
    ...
    - Do NOT preamble non-tool answers. Never say "let me think about
      that", "let me get back to your question", "let me check on the
      context of your question", or similar throat-clearing before
      answering. Just answer directly.

    # False-Wake Handling
    - Your wake word can occasionally fire from background noise — for example, dialogue
      from a TV show or radio, a podcast playing in the room, or a snippet of conversation
      between other people that happens to sound like "Hey Lumina."
    - In those situations, the audio you hear after activation will NOT be a coherent
      request directed at you. It will sound like: a TV/movie scene, two people talking
      to each other (not addressing you), unintelligible mumbling, just ambient noise,
      or a fragment with no clear intent.
    - When that happens, call the dismiss_false_wake tool. Produce NO text and NO audio
      output — only the function call itself. Do not say "sorry, I didn't catch that"
      or "could you repeat that" — just call the tool and stay silent. Like an Echo
      device that lights up but never speaks.
    - Only respond normally when you are reasonably confident the user is speaking to you.
      If you are confident the input is directed at you, proceed as usual — do not
      second-guess legitimate requests.
    """
)

LUMINA_NORMAL_ADDENDUM = ("""
    # Interaction Mode
    - You support three interaction modes: 'normal', 'conversational', and 'strict'.
    - You are currently in 'normal' mode — the microphone is guarded while you are
      speaking, so the user must say the wake word "Hey Lumina" to interrupt you.
      Between turns the user can speak freely without the wake word.
    - In 'conversational' mode, the user can interrupt you mid-sentence by simply
      speaking — no wake word required.
    - In 'strict' mode (also called "wake-word only" mode), the user must say
      "Hey Lumina" before EVERY new request, including follow-ups. The session
      and our conversation history stay intact between turns.
    - "strict mode", "wake-word only mode", "wake word only", and "only respond
      to the wake word" all refer to the SAME mode — pass mode="strict" in
      every case.
    - When the user asks to switch modes, call the set_interaction_mode tool
      with the requested mode, then briefly confirm the change to the user.
    """
)

LUMINA_CONVERSATIONAL_ADDENDUM = ("""
    # Conversational Style
    - Keep responses brief and natural, as the user may interrupt you at any time.
    - Deliver information in small, digestible chunks rather than long monologues.
    - Pause naturally between thoughts to give the user room to interject.

    # Interaction Mode
    - You support three interaction modes: 'normal', 'conversational', and 'strict'.
    - You are currently in 'conversational' mode — the user can interrupt you mid-sentence
      by simply speaking. No wake word is required to interrupt.
    - In 'normal' mode, the microphone is guarded during your speech and the user must
      say "Hey Lumina" to interrupt you.
    - In 'strict' mode (also called "wake-word only" mode), the user must say
      "Hey Lumina" before EVERY new request, including follow-ups.
    - "strict mode", "wake-word only mode", "wake word only", and "only respond
      to the wake word" all refer to the SAME mode — pass mode="strict" in
      every case.
    - When the user asks to switch modes, call the set_interaction_mode tool
...
LUMINA_STRICT_ADDENDUM = ("""
    # Interaction Mode
    - You are currently in 'strict' mode (also called "wake-word only" mode).
      After you finish speaking, the user MUST say "Hey Lumina" again before each new
      request — even short follow-ups. Until they do, anything they say will not reach you.
    - The session and our conversation history stay intact across these wake-word-gated
      turns, so you can reference what we just discussed.
    - You also support 'normal' and 'conversational' modes.
    - "strict mode", "wake-word only mode", "wake word only", and "only respond
      to the wake word" all refer to the SAME mode — pass mode="strict" in
      every case.
    - When the user asks to switch modes, call the set_interaction_mode tool
...
# Personality strings used throughout the rest of the script
LUMINA_NORMAL         = LUMINA_BASE + LUMINA_NORMAL_ADDENDUM
LUMINA_CONVERSATIONAL = LUMINA_BASE + LUMINA_CONVERSATIONAL_ADDENDUM
LUMINA_STRICT         = LUMINA_BASE + LUMINA_STRICT_ADDENDUM
...
_image_client = openai_lib.OpenAI(api_key=OPENAI_API_KEY)
...
INACTIVITY_TIMEOUT = 2   # Lumina times out after this many seconds of inactivity
STRICT_INACTIVITY_TIMEOUT = 30   # Longer window in 'strict' mode so the user can use wake-word for follow-ups
SCREEN_BLANK_TIMEOUT = 600  # Screen blanks after 10 minutes of no wake-word activity unless Always_On is enabled

# "normal"         → wake-word-guarded behavior (mic guarded during AI speech, free between turns)
# "conversational" → OpenAI VAD handles interruption; mic stays active during AI response
# "strict"         → wake-word required before EVERY user turn, including follow-ups
...
# --- Image Size Selection (gpt-image-2 supported sizes) ---
...
def get_best_image_size(width, height):
    """Pick the gpt-image-2 size whose aspect ratio best matches the screen."""
    screen_ratio = width / height
    best_size, best_diff = None, float('inf')
    for size_str, ratio in OPENAI_IMAGE_SIZES:
        diff = abs(screen_ratio - ratio)
        if diff < best_diff:
            best_diff, best_size = diff, size_str
    print(f"Screen {width}x{height} (ratio {screen_ratio:.4f}) -> best image size: {best_size}")
    return best_size
...
def play_chime():
    """Play a two-tone chime three times (1-second apart) using pygame.mixer."""
    ...
    wave = (np.sin(2 * np.pi * 700 * t / sample_rate) # could be 600 or 800
          + np.sin(2 * np.pi * 1050 * t / sample_rate)) # could be 900 or 1200

    envelope = np.ones(num_samples)
    fade_in = int(sample_rate * 0.2)
    fade_out = int(sample_rate * 0.4)
    envelope[:fade_in] = np.linspace(0.0, 1.0, fade_in)
    envelope[-fade_out:] = np.linspace(1.0, 0.0, fade_out)

    wave = (wave / np.max(np.abs(wave)) * envelope * 32767).astype(np.int16)
...
def fire_timer(label):
    ...
    if session_live:
        ...
        if viz is not None:
            subtitle = f"The '{label}' timer has finished." if label != "timer" else ""
            viz.set_text("Timer Finished", subtitle)
...
def generate_openai_image(prompt):
    """Calls gpt-image-2 to generate an image. Returns PIL Image or None."""
    print(f"Sending prompt to gpt-image-2: {prompt}")
    try:
...
def edit_openai_image(edit_prompt, pil_image):
    """Calls gpt-image-2 to edit an existing image. Returns PIL Image or None."""
    print(f"Sending edit prompt to gpt-image-2: {edit_prompt}")
    try:
        buf = io.BytesIO()
        pil_image.save(buf, format="PNG")
        buf.seek(0)
        buf.name = "image.png"
        response = _image_client.images.edit(
            model="gpt-image-2",
            image=buf,
            prompt=edit_prompt,
            size=image_size,
...
def get_current_weather(location=None):
    ...
    Defaults to the device's detected location if no location is specified."""
    if not location:
        q = DEFAULT_WEATHER_Q
...
#        self.screen = pygame.display.set_mode((self.WIDTH, self.HEIGHT), pygame.DOUBLEBUF | pygame.OPENGL)
        ...
        global image_size
        image_size = get_best_image_size(self.WIDTH, self.HEIGHT)
        ...
        self.countdown_font = pygame.font.SysFont("Arial Black", 180)
        self.countdown_label_font = pygame.font.SysFont("Arial Black", 90)
    ...
    def wrap_text_to_width(self, text, font, max_width_px):
        words = text.split()
        if not words:
            return []
        lines = []
        ...
        def append_piece(piece):
            """Add `piece` to the current line if it fits, else start a new line."""
            nonlocal current
        ...
        for word in words:
            if font.size(word)[0] <= max_width_px:
                append_piece(word)
                continue
            # Word is wider than the column on its own — break it up.
            flush()
            for piece in self._break_long_word(word, font, max_width_px):
                if not current:
                    current = piece
    ...
    def _break_long_word(self, word, font, max_width_px):
        """Break `word` into chunks that each fit within max_width_px."""
        BREAK_CHARS = "/_-."
        # First pass: split into pieces, keeping the separator at the end
        # of each piece (so '/home/pi' -> ['/', 'home/', 'pi']).
        pieces = []
        buf = ""
        for ch in word:
            buf += ch
        ...
        for piece in pieces:
            if font.size(piece)[0] > max_width_px:
                # Flush whatever we have, then char-break the oversized piece.
                if cur:
                    chunks.append(cur)
                    cur = ""
                for ch in piece:
    ...
    def display_text(self):
        ...
        VMARGIN = int(self.HEIGHT * 0.05)        # ~5% top/bottom
        HMARGIN = int(self.WIDTH * 0.10)         # ~10% left/right
        COLUMN_WIDTH = self.WIDTH - 2 * HMARGIN
        LINE_SPACING = 50                        # vertical step per body line
        TITLE_BODY_GAP = 40                      # gap between title and body

        # --- Title ---
        title_surface = None
        title_height = 0
        if self.display_text_line1:
            title_surface = self.text_font.render(
                self.display_text_line1, True, self.TEXT_COLOR
            )
            title_height = title_surface.get_height()

        # --- Body word-wrap (pixel-based) ---
        body_lines = []
        if self.display_text_line2:
            body_lines = self.wrap_text_to_width(
                self.display_text_line2, self.text_font, COLUMN_WIDTH
            )

        # --- Truncate if overflowing ---
        available_height = self.HEIGHT - 2 * VMARGIN
        title_block_height = title_height + (
            TITLE_BODY_GAP if title_height and body_lines else 0
        )
        max_body_height = max(0, available_height - title_block_height)
        max_body_lines = max_body_height // LINE_SPACING if LINE_SPACING else 0
        ...
        total_height = title_block_height + body_height
        start_y = (self.HEIGHT - total_height) // 2

        # --- Draw title (centered horizontally) ---
        y = start_y
        if title_surface:
            rect = title_surface.get_rect(midtop=(self.WIDTH // 2, y))
            self.ui_surface.blit(title_surface, rect)
            y += title_height + TITLE_BODY_GAP

        # --- Draw body lines (left-aligned within centered column) ---
        body_left = HMARGIN
        for line in body_lines:
            surf = self.text_font.render(line, True, self.TEXT_COLOR)
            self.ui_surface.blit(surf, (body_left, y))
            y += LINE_SPACING
...
class AudioIO:
    ...
    def _process_guarded_audio(self, raw_24k_bytes):
        ...
        while len(self._porcupine_buf) >= porc_fl:
            ...
            if keyword_index == 0:  # "Hey Lumina" detected
...
class Realtime:
    def __init__(self, api_key, ws_url, p_instance, visualizer_instance, porcupine):
        ...
        self.dismissed_silently = False
        self.interaction_mode = INTERACTION_MODE
        self.wake_word_re_armed = False
    ...
    def start(self):
        ...
        self.audio.set_mode('active')  # User just said "Hey Lumina", expect speech
    ...
    def handle_wake_word_interrupt(self):
        ...
        print("Hey Lumina detected - interrupting AI and switching to active mode")

        self.wake_word_re_armed = True

        if self.visualizer is not None and self.visualizer.state != 'blanked':
            self.visualizer.state = 'visualizing'
    ...
    def _silently_dismiss(self, reason="unknown"):
        print(f"Silent dismissal — reason: {reason}")
        if not self.dismissed_silently:
            self.audio.clear_buffer()
            try:
                self.sock.send({"type": "response.cancel"})
            except Exception as e:
                print(f"response.cancel during dismissal failed: {e}")
        self.dismissed_silently = True

    def _apply_mode_switch(self, new_mode):
        global INTERACTION_MODE
        if new_mode not in ("normal", "conversational", "strict"):
            return f"Invalid mode '{new_mode}'. Please choose 'normal', 'conversational', or 'strict'."

        if new_mode == self.interaction_mode:
            return f"Already in {new_mode} mode — no change needed."

        old_mode = self.interaction_mode
        self.interaction_mode = new_mode
        INTERACTION_MODE = new_mode
        print(f"Interaction mode: {old_mode} -> {new_mode}")

        if new_mode == "conversational":
            instructions = LUMINA_CONVERSATIONAL
        elif new_mode == "strict":
            instructions = LUMINA_STRICT
        else:
            instructions = LUMINA_NORMAL
        ...
        # Partial session.update — only fields that depend on mode.
        self.sock.send({
            "type": "session.update",
            "session": {
                "type": "realtime",
                ...
                "audio": {
                    "input": {
                        "turn_detection": {
                            "type": "server_vad",
                            "threshold": 0.80 if new_mode == "conversational" else 0.88,
                            "silence_duration_ms": 550 if new_mode == "conversational" else 700,
        ...
        return f"Switched from {labels[old_mode]} to {labels[new_mode]}."
    ...
    def on_msg(self, msg):
        ...
        if typ == "response.output_item.added":
            ...
            if (item.get("type") == "function_call"
                    and item.get("name") == "dismiss_false_wake"):
                self._silently_dismiss(reason="model called dismiss_false_wake")
                return
        ...
        if typ == "session.created" and not self.sent_update:
            ...
            # Build instructions, selecting personality by current interaction_mode
            if self.interaction_mode == "conversational":
                instructions = LUMINA_CONVERSATIONAL
            elif self.interaction_mode == "strict":
                instructions = LUMINA_STRICT
            else:
                instructions = LUMINA_NORMAL
            if self.visualizer.last_generated_pil_image is not None:
                ...
                "session": {
                    ...
                    "tools": [
                        ...
                        },
                        {
                            ...
                            "name": "edit_image",
                            "description": (
                                "Edit or modify the currently displayed image based on a description "
                                "of the desired changes. Call this when the user asks to change, adjust, "
                                "edit, or modify the current image (e.g., 'make the sky more orange', "
                                "'add a cat', 'remove the tree'). Each edit builds on the current image. "
                                "Do not call this if no image is currently displayed — use generate_image instead."
                            ),
                            "parameters": {
                                "type": "object",
                                "properties": {
                                    "edit_prompt": {
                                        "type": "string",
                                        "description": (
                                            "A description of the changes to make to the current image. "
                        ...
                        {
                            "type": "function",
                            "name": "dismiss_false_wake",
                            "description": (
                                "Call this when the audio you hear after activation is NOT a "
                                "coherent request directed at you — for example, dialogue from "
                                "a TV show or radio, background conversation between other "
                                "people, unintelligible mumbling, or just ambient noise. "
                                "When you call this tool, produce NO text and NO audio output "
                                "— only the function call itself. Do not apologize, do not say "
                                "'sorry, I didn't catch that' — just call the tool and stay "
                                "silent. The system will close the session quietly, like an "
                                "Echo device that lights up but never speaks."
                            ),
                            "parameters": {
                                "type": "object",
                                "properties": {},
                        ...
                        },
                        {
                            ...
                            "description": (
                                "Switches the interaction mode between 'normal', 'conversational', "
                                "and 'strict'. "
                                "Use 'normal' for the standard wake-word-guarded mode (free speech "
                                "between turns, wake word interrupts AI). "
                                "Use 'conversational' to allow the user to interrupt Lumina while "
                                "speaking. "
                                "Use 'strict' to require the wake word before EVERY user turn, "
                                "including follow-ups; the session and history stay alive. "
                                "Important: 'strict mode', 'wake-word only mode', 'wake word only', "
                                "and 'only respond to the wake word' all refer to the SAME mode — "
                                "pass mode='strict' in every case."
                            ),
                            "parameters": {
                                "type": "object",
                                "properties": {
                                    "mode": {
                                        "type": "string",
                                        "enum": ["normal", "conversational", "strict"],
                                        "description": "The interaction mode to switch to."
                                    }
                                },
                                "required": ["mode"],
                                "additionalProperties": False
                    ...
                    "audio": {
                        "input": {
                            "turn_detection": {
                                "type": "server_vad",
                                "threshold": 0.80 if self.interaction_mode == "conversational" else 0.88,
                                "silence_duration_ms": 550 if self.interaction_mode == "conversational" else 700,
                                "prefix_padding_ms": 180,
                                "idle_timeout_ms": 15000
                            }
                        },
        ...
        # interrupt in guarded mode.
        ...
        elif typ == "input_audio_buffer.timeout_triggered":
            print("Idle timeout triggered by server VAD")
            ...
            if self.interaction_mode != "conversational":
                ...
                self.sock.send({"type": "input_audio_buffer.clear"})
            threading.Thread(target=set_alsa_capture, args=(33,), daemon=True).start()
            ...
            if status == "completed" and not has_function_call:
                ...
                threading.Thread(target=set_alsa_capture, args=(42,), daemon=True).start()
    ...
    def execute_tool(self, call_id, func_name, raw_args):
        ...
        if func_name == "dismiss_false_wake":
            self._silently_dismiss(reason="model called dismiss_false_wake (late path)")
            return
        ...
        try:
            if func_name == "get_current_time":
                ...
                try:
                    ...
                    pil_image = generate_openai_image(prompt)
                ...
                if self.visualizer.last_generated_pil_image is None:
                    output = "No image is currently displayed. Ask the user to generate an image first."
                else:
                    self.generating_image = True
                    try:
                        self.visualizer.set_text("Editing image...", edit_prompt)
                        print(f"Editing image with prompt: {edit_prompt}")
                        pil_image = edit_openai_image(edit_prompt, self.visualizer.last_generated_pil_image)
                        if pil_image:
                            self.visualizer.set_image(pil_image, edit_prompt)
                            self.new_image_this_session = True
                            output = f"Image edited successfully: {edit_prompt}"
                            print("Edited image displayed.")
                        else:
                            self.visualizer.set_text("Image edit failed.", "Please try again.")
                            output = "Image edit failed. The API returned no image. Tell the user to try again."
                    except Exception as e:
                        error_msg = str(e)
                        self.visualizer.set_text("An error occurred.", error_msg[:80])
                        output = f"Image edit failed with error: {error_msg}"
                    finally:
                        self.generating_image = False
            ...
            elif func_name == "set_interaction_mode":
                output = self._apply_mode_switch(args.get("mode"))
                ...
                if keyword_index == 0:
                    ...
                    try:
                        ...
                        while True:
                            ...
                            if rt_session.wake_word_re_armed:
                                rt_session.wake_word_re_armed = False
                                last_activity_time = time.time()
                                if visualizer.state != 'blanked':
                                    visualizer.state = 'visualizing'

                            if (rt_session.interaction_mode == "strict"
                                    and not ai_is_talking
                                    and not user_is_talking
                                    and not rt_session.generating_image
                                    and visualizer.state == 'visualizing'
                            ...
                            # Silent dismissal of a false wake-word — close session quietly
                            if rt_session.dismissed_silently:
                                print("Silently dismissed false wake. Closing session quietly.")
                                break

                            timeout = (STRICT_INACTIVITY_TIMEOUT
                                       if rt_session.interaction_mode == "strict"
                        ...
                        dismissed_silently = rt_session.dismissed_silently

                        rt_session.stop()

                        # --- Post-session screen behavior (priority order) ---
                        if dismissed_silently:
                            if was_blanked_at_wake:
                                visualizer.state = 'blanked'
                                time.sleep(0.05)
                                blank_screen()
                                screen_blanked = True
...
def main():
    ...
    # Run 'amixer scontrols' to verify control names for USB speakerphone.
    playback_ok = set_alsa_playback(current_volume)
    capture_ok = set_alsa_capture(48)
    print(f"ALSA playback (PCM {current_volume}%): {'OK' if playback_ok else 'FAILED'}")
    print(f"ALSA capture  (Mic 48%): {'OK' if capture_ok else 'FAILED'}")
